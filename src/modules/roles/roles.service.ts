...
export class RolesService {
    ...
    async createRole(dto: CreateRoleDto) {
        ...
        if (dto.permissions?.length) {
            // Map the string IDs to RolePermission entities
            rolePermissions = dto.permissions.map((id) => {
                const rp = new RolePermissionEntity();
                // We set the permission as a partial object with just the ID
                // TypeORM is smart enough to use this for the foreign key
    ...
    async findOneRole(id: string) {
        const role = await this.rolesRepository.findOne({ where: { id }, relations: ['rolePermissions', 'rolePermissions.permission'] });
        if (!role) throw new NotFoundException(`Role with ID ${id} not found`);
        return role;
    }
    ...
    async createPermission(dto: NestedPermissionDto): Promise<PermissionEntity> {
        ...
        if (existing) {
            throw new ConflictException(`Permission for module "${dto.module}" and action "${dto.action}" already exists.`)
        }
